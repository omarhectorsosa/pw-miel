...
test('test', async ({ page }) => {
  ...
  await page.getByRole('link', { name: 'DOSANTOS, MARÍA LAURA' }).click();
  await page.getByRole('link', { name: '[CORREGIR]' }).first().click();
  await page.getByRole('textbox', { name: 'Observaciones (opcional):' }).click();
  await page.getByRole('textbox', { name: 'Observaciones (opcional):' }).fill('Hola hay que entregar ');
  await page.getByRole('textbox', { name: 'Observaciones (opcional):' }).fill('Hola hay que entregar ');
});